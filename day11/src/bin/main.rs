...
impl Location {
    pub fn new(x: f64, y: f64, z: f64, area: f64, snow: impl Into<Snowball>) -> Location {
        let snow = snow.into();
        Location {
            x,
            y,
    ...
    // - z: f64
    // - area: f64
    // - snow: Either `SnowKg`, `SnowLb` or `Snowball` -- Handled by using the impl Into<Snowball> type definition which includes all types that implement From and therefore Into automatically
...
pub fn find_best_location(locations: Vec<Location>) -> Result<Location, Box<dyn Error>> {
    let best = locations
        ...
        .ok_or("Highest density location not found");
    return Ok(best?);
